...
void ABossEnemy1::SpawnAOE()
{
    ...
    const float SpawnRadius = 300.0f; // 랜덤 범위 (반지름)
        ...
        TimerDel.BindWeakLambda(this, [this, CenterLocation, SpawnRadius]()
                                {
                                    ...
                                    if (bUsePooling)
                                    {
                                        ...
                                        {
                                            if (UPoolManagerSubsystem *PoolMgr = World->GetSubsystem<UPoolManagerSubsystem>())
                                            {
                                                PoolMgr->SpawnFromPool(this->AOEExplosionClass, FinalSpawnLocation, FRotator::ZeroRotator);
                                            }
                                        }
                                    }
                                    else
